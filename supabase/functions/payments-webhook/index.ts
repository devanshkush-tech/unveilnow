...
async function syncProfilePlan(userId: string, priceId: string | null, status: string) {
  const isActive = status === "active" || status === "trialing" || status === "past_due";
  const plan = isActive && priceId && PLAN_FROM_PRICE[priceId] ? PLAN_FROM_PRICE[priceId] : "free";
...
async function handleSubscriptionDeleted(subscription: any, env: StripeEnv) {
  await getSupabase()
    ...
    .eq("environment", env);
  const userId = subscription.metadata?.userId;
  if (userId) await syncProfilePlan(userId, null, "canceled");
}
